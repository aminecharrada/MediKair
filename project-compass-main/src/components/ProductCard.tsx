    ...
    >
      <div className="relative aspect-square overflow-hidden bg-muted">
        ...
        {product.badge && (
          <span className={`absolute left-2 top-2 rounded-md px-1.5 py-0.5 text-[10px] font-semibold sm:left-3 sm:top-3 sm:px-2 sm:text-xs ${
            ...
            : product.badge === "IA Recommandé" ? "bg-secondary text-secondary-foreground"
            : "bg-primary text-primary-foreground"
          }`}>{product.badge}</span>
        )}
        {!product.inStock && (
          <div className="absolute inset-0 flex items-center justify-center bg-background/60">
            <span className="rounded-md bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground sm:px-3 sm:py-1 sm:text-sm">Rupture de stock</span>
          </div>
        )}
      </div>
      <div className="flex flex-1 flex-col p-3 sm:p-4">
        ...
        <div className="mt-auto flex items-end justify-between pt-2 sm:pt-3">
          <div>
            <span className="font-display text-sm font-bold text-foreground sm:text-lg">{product.price.toFixed(2)} MAD</span>
            {product.oldPrice && <span className="ml-1 text-[10px] text-muted-foreground line-through sm:ml-2 sm:text-xs">{product.oldPrice.toFixed(2)}</span>}
          </div>
          <Button size="icon" variant="outline" className="h-7 w-7 shrink-0 sm:h-8 sm:w-8" onClick={(e) => { e.preventDefault(); }}>
