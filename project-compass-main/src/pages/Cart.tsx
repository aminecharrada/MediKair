        ...
        {cart.length === 0 ? (
          ...
          <div className="mt-6 flex flex-col gap-6 lg:flex-row lg:gap-8">
            ...
            <div className="flex-1 space-y-3 sm:space-y-4">
              {items.map((item, i) => (
                <motion.div key={item.productId} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  className="flex gap-3 rounded-xl border border-border bg-card p-3 shadow-card sm:gap-4 sm:p-4">
                  <img src={item.product.image} alt={item.product.name} className="h-16 w-16 shrink-0 rounded-lg object-cover sm:h-20 sm:w-20" />
                  <div className="flex flex-1 flex-col min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="text-[10px] text-muted-foreground sm:text-xs">{item.product.brand}</p>
                        <Link to={`/produit/${item.product.id}`} className="text-xs font-semibold hover:text-secondary transition-colors line-clamp-2 sm:text-sm">
                          {item.product.name}
                        </Link>
                      </div>
                      <button onClick={() => remove(item.productId)} className="shrink-0 text-muted-foreground hover:text-destructive transition-colors">
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
                    <div className="mt-auto flex items-center justify-between pt-2">
                      <div className="flex items-center gap-1.5 rounded-lg border border-border sm:gap-2">
                        <button onClick={() => updateQty(item.productId, -1)} className="p-1 text-muted-foreground hover:text-foreground transition-colors sm:p-1.5">
              ...
              <div className="sticky top-20 rounded-xl border border-border bg-card p-5 shadow-card sm:p-6">
                ...
                <dl className="mt-4 space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <dd className="font-medium">{subtotal.toFixed(2)} MAD</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">Livraison</dt>
                    <dd className="font-medium">{shipping === 0 ? "Gratuite" : `${shipping.toFixed(2)} MAD`}</dd>
                  </div>
                  {shipping > 0 && <p className="text-xs text-secondary">Livraison gratuite à partir de 500 MAD</p>}
                  <div className="border-t border-border pt-3 flex justify-between">
                    <dt className="font-semibold">Total TTC</dt>
                    <dd className="font-display text-lg font-extrabold sm:text-xl">{total.toFixed(2)} MAD</dd>
                  </div>
                </dl>
                <Link to="/checkout">
                  <Button className="mt-5 w-full bg-hero-gradient text-primary-foreground hover:opacity-90 font-semibold" size="lg">
                    Passer la commande <ArrowRight className="ml-2 h-4 w-4" />
                  </Button>
                </Link>
                <Link to="/catalogue">
                  <Button variant="ghost" className="mt-2 w-full" size="sm">Continuer mes achats</Button>
                </Link>
              </div>
            </div>
          </div>
